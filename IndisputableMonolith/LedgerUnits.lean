...
/-- WIP axiom: Right-cancellation on ℤ (domain) -/
axiom int_mul_right_cancel {a b c : ℤ} (hc : c ≠ 0) : a * c = b * c → a = b

lemma rep_unique {δ n m : ℤ} (hδ : δ ≠ 0) (h : n * δ = m * δ) : n = m := by
  -- WIP axiom: right-cancellation for integers
  exact int_mul_right_cancel hδ h
...
end IndisputableMonolith.LedgerUnits
