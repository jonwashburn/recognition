...
/-- Predicate that a certification family has no certificates at all. -/
def isEmpty (C : CertFamily) : Prop :=
  C.units = [] ∧ C.eightbeat = [] ∧ C.elprobes = [] ∧ C.masses = [] ∧ C.rotation = [] ∧ C.outer = [] ∧ C.conscious = []

lemma verified_of_isEmpty (φ : ℝ) {C : CertFamily} (h : isEmpty C) : Verified φ C := by
  rcases h with ⟨hu, he8, hel, hm, hrot, hout, hcons⟩
  dsimp [Verified]
  refine And.intro ?hU (And.intro ?hE8 (And.intro ?hEL (And.intro ?hM (And.intro ?hRot (And.intro ?hOut ?hCons)))))
  all_goals
    intro x hx
  ...
  · simpa [hrot] using hx
  · simpa [hout] using hx
  · simpa [hcons] using hx

/-- Total size (count) of all certificates in a family. -/
@[simp] def size (C : CertFamily) : Nat :=
  unitsCount C + eightbeatCount C + elprobesCount C + massesCount C + rotationCount C + outerCount C + consciousCount C

lemma size_append (C₁ C₂ : CertFamily) :
  size (append C₁ C₂) = size C₁ + size C₂ := by
  dsimp [size, append, unitsCount, eightbeatCount, elprobesCount, massesCount, rotationCount, outerCount, consciousCount]
  simp [List.length_append, Nat.add_comm, Nat.add_left_comm, Nat.add_assoc]

@[simp] lemma size_emptyFamily : size emptyFamily = 0 := by
  dsimp [size, emptyFamily, unitsCount, eightbeatCount, elprobesCount, massesCount, rotationCount, outerCount, consciousCount]
  simp

@[simp] lemma size_consMass (c : MassCert) (C : CertFamily) :
  size (consMass c C) = size C + 1 := by
  dsimp [size, consMass, unitsCount, eightbeatCount, elprobesCount, massesCount, rotationCount, outerCount, consciousCount]
  simp

@[simp] lemma size_consUnits (c : UnitsCert) (C : CertFamily) :
  size (consUnits c C) = size C + 1 := by
  dsimp [size, consUnits, unitsCount, eightbeatCount, elprobesCount, massesCount, rotationCount, outerCount, consciousCount]
  simp

@[simp] lemma size_consEightBeat (c : EightBeatCert) (C : CertFamily) :
  size (consEightBeat c C) = size C + 1 := by
  dsimp [size, consEightBeat, unitsCount, eightbeatCount, elprobesCount, massesCount, rotationCount, outerCount, consciousCount]
  simp

@[simp] lemma size_consELProbe (c : ELProbe) (C : CertFamily) :
  size (consELProbe c C) = size C + 1 := by
  dsimp [size, consELProbe, unitsCount, eightbeatCount, elprobesCount, massesCount, rotationCount, outerCount, consciousCount]
  simp

@[simp] lemma size_consRotation (c : RotationCert) (C : CertFamily) :
  size (consRotation c C) = size C + 1 := by
  dsimp [size, consRotation, unitsCount, eightbeatCount, elprobesCount, massesCount, rotationCount, outerCount, consciousCount]
  simp

@[simp] lemma size_consOuter (c : OuterBudgetCert) (C : CertFamily) :
  size (consOuter c C) = size C + 1 := by
  dsimp [size, consOuter, unitsCount, eightbeatCount, elprobesCount, massesCount, rotationCount, outerCount, consciousCount]
  simp

@[simp] lemma size_consConscious (c : ConsciousCert) (C : CertFamily) :
  size (consConscious c C) = size C + 1 := by
  dsimp [size, consConscious, unitsCount, eightbeatCount, elprobesCount, massesCount, rotationCount, outerCount, consciousCount]
  simp
...
/-- Merge a list of verified generator packs at fixed φ. -/
def VerifiedGenerators.mergeAll {φ : ℝ} (gs : List (VerifiedGenerators φ)) : VerifiedGenerators φ :=
{ C := foldAppend (gs.map (fun g => g.C))
, ok := by
    -- Prove verification of foldAppend by mapping proofs over the list
    have : ∀ C ∈ (gs.map (fun g => g.C)), Verified φ C := by
      intro C hC
      rcases List.mem_map.mp hC with ⟨g, hg, rfl⟩; exact g.ok
    simpa [foldAppend] using verified_foldl_append φ (gs.map (fun g => g.C)) this }
